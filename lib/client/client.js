...
var zendeskAgent = new https.Agent({
  ...
  /**
   * Optimization.  Note that sockets are destroyed if not in use.  From the HTTP docs:
...
Client.prototype.request = function (method, uri) {
  ...
  return this._request(options, function (err, response, result) {
    ...
    try {
      ...
      // Emit Errors (sometimes zendesk returns HTML instead of a JSON blob)

      // This result object often goes to loggers, which result in ugly JSON stringifications of a byte array.
      // If the result is a byte array, transform it back to text (often an HTML response appears, hence parse error)
      var resultStr;
      if (result instanceof Buffer) {
        resultStr = String.fromCharCode.apply(null, result);
      }

      self.emit('debug::error', {exception: ex, code: statusCode, request: options, result: resultStr || result});
    }
...
Client.prototype.requestN = function (method, uri) {
  var args         = Array.prototype.slice.call(arguments),
      callback     = args.pop(),
      numberRecords= args.pop(),
      nextPage     = 'Not Null!',
      bodyList     = [],
      statusList   = [],
      responseList = [],
  ...
  return Client.prototype.request.apply(this, args.concat(function (error, status, body, response, result) {
    ...
    async.whilst(
      function () {return ((nextPage !== null && typeof nextPage !== 'undefined') && numberRecords > flatten(bodyList).length); },
      function (cb) {
